...
for state in state_data:
    ...
    for row in university_rows:
        ...
        if university_link_tag:
            ...
            # Extract town name from another <td> in the same row
            #town_name = row.select_one('td:nth-of-type(2)').get_text(strip=True)
            town_name = soup.find('span', itemprop='addressLocality').get_text() if soup.find('span',
                                                                                              itemprop='addressLocality') else None




            # Step 3: Visit the university page to scrape more data (logo, social media, etc.)
